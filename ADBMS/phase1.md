...
### On Windows (with MinGWg++)
Ensure `g++` is in your PATH, and run
```powershell
g++ src.cpp -I srcinclude -std=c++17 -o qopt.exe
.qopt.exe
```
...
- `load_from_csv()` Opens the CSV files, parses the delimited data into typed `Row`s, and calculates base statistics (row count, minmax values, distinct counts). 
...
- Each `execute_` function processes input rows and returns a completely materialized `ExecResult` (containing the new `TableSchema` and the `stdvectorRow`).
