...
std::string pred_to_string(const Predicate& p) {
    ...
    if (p.is_col1) {
        ...
        if (p.literal1.type == DataType::TEXT) s += "'" + std::get<std::string>(p.literal1.data) + "'";
        else if (p.literal1.type == DataType::INT) s += std::to_string(std::get<int>(p.literal1.data));
        else if (p.literal1.type == DataType::DOUBLE) s += std::to_string(std::get<double>(p.literal1.data));
    }
    s += " " + op_to_string(p.op) + " ";
    if (p.is_col2) {
